```meta-bind-js-view
{HwayoungMiniSheetConfig#conMod} as conMod
{HwayoungMiniSheetConfig#dexMod} as dexMod
{HwayoungMiniSheetConfig#wisMod} as wisMod
{HwayoungMiniSheetConfig#chaMod} as chaMod
{Adarin Mini Sheet#monkunchainedLevel} as monkLevel
{Adarin Mini Sheet#paladinLevel} as paladinLevel
...
save to {HwayoungMiniSheetConfig#saves}
hidden
---
...
const fortSave = baseFort + conMod + resistanceEnhancement + (conditionEffects.fortAdjust || 0);
const refSave = baseRef + dexMod + resistanceEnhancement + (conditionEffects.refAdjust || 0);
const willSave = baseWill + wisMod + resistanceEnhancement + (conditionEffects.willAdjust || 0);

// Create notes as arrays for each save
const fortSaveNotesArray = [];
const refSaveNotesArray = [];
const willSaveNotesArray = [];
...
`VIEW[{HwayoungMiniSheetConfig#saves.fort.value}][text(class(fort-save))]` `VIEW[{HwayoungMiniSheetConfig#saves.fort.notes}][text(renderMarkdown, class(save-note fort))]`  `VIEW[{HwayoungMiniSheetConfig#saves.ref.value}][text(class(ref-save))]` `VIEW[{HwayoungMiniSheetConfig#saves.ref.notes}][text(renderMarkdown, class(save-note ref))]` `VIEW[{HwayoungMiniSheetConfig#saves.will.value}][text(class(will-save))]` `VIEW[{HwayoungMiniSheetConfig#saves.will.notes}][text(renderMarkdown, class(save-note will))]`